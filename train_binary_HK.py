'''
learning_rate=0.1, nkerns=[6, 14], batch_size=70, ktop=4, kmax=30, dropout=0.2, hidden units=50, L1=0, L2=0.000005, useEmb=1, performance=20.000000%
learning_rate=0.1, nkerns=[6, 14], batch_size=100, ktop=4, kmax=30, dropout=0.2, hidden units=50, L1=0, L2=0.000005, useEmb=1, performance=20.777778%
#useAllSamples=1
learning_rate=0.1, nkerns=[6, 14], batch_size=50, ktop=4, kmax=30, dropout=0.2, hidden units=50, L1=0, L2=0.000005, useEmb=1, performance=21.135135%
learning_rate=0.1, nkerns=[6, 14], batch_size=70, ktop=4, kmax=30, dropout=0.2, hidden units=50, L1=0, L2=0.000005, useEmb=1, performance=23.121693%
useAllSamples=0
learning_rate=0.1, nkerns=[6, 14], batch_size=70, ktop=4, kmax=30, dropout=0.2, hidden units=50, L1=0, L2=0.000005, useEmb=1, performance=21.208791%

# add fold for the first layer
2) learning_rate=0.1, nkerns=[6, 14], batch_size=70, ktop=4, kmax=30, dropout=0.2,L1=0, L2=0.000005, useEmb=0, performance=19.230769%
3) learning_rate=0.1, nkerns=[6, 14], batch_size=100, ktop=4, kmax=30, dropout=0.2,L1=0, L2=0.000005, useEmb=0, performance=19.666667%
...
from HKDefined import ConvFoldPoolLayer,Conv_DynamicK_PoolLayer, dropout_from_layer, shared_dataset, read_data_HK
from collections import OrderedDict


def evaluate_lenet5(learning_rate=0.1, n_epochs=2000, nkerns=[6, 14], batch_size=10, useAllSamples=0, kmax=30, ktop=4, filter_size=[7,5],
                    L2_weight=0.000005, dropout_p=0.2, useEmb=0, task=2, corpus=1):
    ...
    datasets, embedding_size, embeddings=read_data_HK(root+str(task)+'classes/'+str(corpus)+'train.txt', root+str(task)+'classes/'+str(corpus)+'dev.txt', root+str(task)+'classes/'+str(corpus)+'test.txt', embeddingPath,60, useEmb)
    ...
    layer1 = ConvFoldPoolLayer(rng, input=layer0.output,
            ...
            filter_shape=(nkerns[1], nkerns[0], filter_size2[0], filter_size2[1]), poolsize=poolsize2, k=ktop, dynamicK=layer0.k)
    ...
    '''
    dropout=dropout_from_layer(rng, layer2_input, dropout_p)               #dropout
    ...
    validation_frequency = min(n_train_batches/30, patience / 2)
