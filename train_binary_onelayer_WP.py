...
'''
learning_rate=0.1, nkerns=[6, 14], batch_size=100, ktop=4, kmax=30, dropout=0.8, L1=0, L2=0.000005, useEmb=0, performance=18.166667%
'''
import cPickle
import gzip
import os
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=2000, nkerns=[6, 14], batch_size=100, useAllSamples=1, kmax=30, ktop=4, filter_size=[7,5],
                    L2_weight=0.00001, dropout_p=0.8, useEmb=0, task=2):
    ...
    datasets, embedding_size, embeddings=read_data_WP(root+str(task)+'classes/train.txt', root+str(task)+'classes/dev.txt', root+str(task)+'classes/test.txt', embeddingPath,60, useEmb)
    ...
    layer0 = Conv_Fold_DynamicK_PoolLayer(rng, input=layer0_input,
            image_shape=(batch_size, 1, ishape[0], ishape[1]),
            filter_shape=(nkerns[0], 1, filter_size1[0], filter_size1[1]), poolsize=poolsize1, k=dynamic_lengths, unifiedWidth=kmax, left=left_after_conv, right=right_after_conv)
    ...
    layer1 = ConvFoldPoolLayer(rng, input=layer0.output,
            image_shape=(batch_size, nkerns[0], ishape[0], kmax),
            filter_shape=(nkerns[1], nkerns[0], filter_size2[0], filter_size2[1]), poolsize=poolsize2, k=ktop, left=layer0.leftPad, right=layer0.rightPad)
    '''
    ...
    layer2 = FullyConnectedLayer(rng, input=dropout, n_in=nkerns[0] * (embedding_size/2) * kmax, n_out=task, useTanh=False)
    layer3=SoftMaxlayer(input=layer2.output)
    #layer3 = LogisticRegression(rng, input=dropout, n_in=nkerns[0] * (embedding_size/2) * kmax, n_out=task)
    ...
    L2_reg = (layer2.W** 2).sum()+(layer0.W** 2).sum()+(embeddings**2).sum()
    ...
    params = layer2.params  + layer0.params+[embeddings]
