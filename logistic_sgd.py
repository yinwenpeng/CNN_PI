...
def read_data(trainFile, devFile, testFile, emb_file, maxlength):
    ...
    def load_file(file):   
        ...
        for line in senti_file:
            ...
            left=(maxlength-len(words))/2
            right=maxlength-left-len(words)
    ...
    devX, devY=load_file(devFile)
    print 'dev file loaded over'
    testX, testY=load_file(testFile)
    print 'test file loaded over'
    '''
    def check_length(matrix):
        last_length=len(matrix[0])
        for i in range(1,len(matrix)):
            if len(matrix[i]) != last_length:
                print 'different length at '+str(i)+' '+str(len(matrix[i]))+'vs'+str(last_length)
                exit(0)
            else:
                last_length=len(matrix[i])
    check_length(trainX)
    check_length(devX)
    check_length(testX)
    print 'check length over'
    exit(0)
    '''
    ...
    test_set_x, test_set_y = shared_dataset(testX, testY)
    valid_set_x, valid_set_y = shared_dataset(devX, devY)
    train_set_x, train_set_y = shared_dataset(trainX, trainY)

    rval = [(train_set_x, train_set_y), (valid_set_x, valid_set_y),
            (test_set_x, test_set_y)]
    return rval,      embedding_size
